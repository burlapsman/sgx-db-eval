...
#define KB 64
#define BLOCK_NUM 1*KB
#define MAX_BUF_LEN (BLOCK_NUM*BLOCK_SIZE)
...
typedef unsigned int/*long*/ uint32;         /* Unsigned 32 bit value */
...
int main() {
	...
	printf("1. to run AES-128 in enclave.\n");
	printf("2. to run AES-128 out of enclave.\n");
	scanf("%d", &ch);
...
int test_enclave() {
	...
	uint8 *buffer = (uint8*)malloc(MAX_BUF_LEN);
	...
	*/
	...
	// ecall
	foo(eid, (char*)buffer, MAX_BUF_LEN, s);
...
void bar(char* buf, size_t len, size_t num) {
	uint8 k[16] = { 0x1a, 0x2b, 0x3b, 0x11, 0x1a, 0x2b, 0x3b, 0x11, 0x1a, 0x2b, 0x3b, 0x11, 0x1a, 0x2b, 0x3b, 0x11 };
	uint8 list[16];
	int n = len >> 4; // len/16
	char* buf_const = buf;
